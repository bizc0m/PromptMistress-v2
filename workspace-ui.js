...
function render(){
 ...
 if(active&&!visible.some(r=>r.key===active.key)){retainDraft();active=null;dirty=false;detail=null;serial++;$('record').hidden=true;$('empty').hidden=false;$('body').textContent='Choisis une conversation ou un prompt.';$('body-fold').open=false;}
 ...
 for(const r of visible.slice(0,limit)){
  ...
  const date=cell();date.className='date-cell';const d=new Date(r.updated);date.textContent=r.updated&&!Number.isNaN(d.valueOf())?d.toLocaleString('fr-FR',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}):'—';date.title=r.updated||'Date inconnue';
  ...
  const project=document.createElement('span');project.className='vault-badge project';project.textContent=r.project||'—';cell().append(project);
  const status=document.createElement('span');status.className='vault-badge status';status.textContent=a.visibility==='archived'?'Archivé':a.visibility==='hidden'?'Masqué':a.status||'—';cell().append(status);
 ...
 $('counts').textContent=(booleanPending?'Recherche en cours…':format(visible.length)+' résultats')+' · '+format(rows.filter(r=>r.kind==='chat').length)+' conversations · '+format(rows.filter(r=>r.kind==='prompt').length)+' prompts';
}
function selectionCount(){$('selected-count').textContent=selected.size?format(selected.size)+' sélectionné(s)':'';document.querySelector('.selection').hidden=false;$('export-selection').disabled=!selected.size;$('export-selection').textContent='Exporter la sélection ('+format(selected.size)+')';document.querySelector('.bulk').hidden=!selected.size;const all=$('select-all-rows');if(all){const n=visible.filter(r=>selected.has(r.key)).length;all.checked=!!visible.length&&n===visible.length;all.indeterminate=n>0&&n<visible.length;}}
function fillAnnotation(){const a={...annotation(active),...(drafts[active.key]||{})};dirty=!!drafts[active.key];$('record-tags').replaceChildren();addBadges($('record-tags'),a.tags||[]);$('visibility-label').textContent=a.visibility==='hidden'?'Masqué':a.visibility==='archived'?'Archivé':'Actif';for(const key of ['tags','folders','note','status','priority','color'])$(key).value=Array.isArray(a[key])?a[key].join(', '):a[key]||'';$('annotation-status').textContent=dirty?'Brouillon conservé · à enregistrer':'';}
...
async function readDetail(){
 ...
 for(const id of ['copy','highlight','clear-highlights'])$(id).disabled=true;
 ...
 for(const id of ['copy','highlight','clear-highlights'])$(id).disabled=false;
...
function refreshState(state){
 ...
 button.textContent=state==='fresh'?'✓ À jour · Actualiser':state==='loading'?'↻ Relecture…':'↻ À actualiser';
...
function renderExplorer(){
 ...
 $('project-filter').replaceChildren(new Option('Tous les projets',''));
...
async function loadCorpus(){
 ...
 corpusPromise=(async()=>{const documents=[];let offset=0,failed=0,total=0;
  ...
  corpus=documents;$('corpus-status').textContent='Texte intégral disponible : '+documents.length+' / '+total+' versions'+(failed?' · '+failed+' erreur(s) de lecture':'')+'. Couverture limitée aux sources chargées.';return documents;
...
let views={},viewsValid=true;try{views=JSON.parse(localStorage.getItem(viewsKey)||'{}');if(!views||typeof views!=='object'||Array.isArray(views))throw Error();}catch{viewsValid=false;notify('Vues enregistrées illisibles : stockage conservé.');}
function renderViews(){const previous=$('saved-views').value;$('saved-views').replaceChildren(new Option('Choisir une vue',''));for(const name of Object.keys(views))$('saved-views').append(new Option(name,name));$('saved-views').value=previous;}
...
const ruleChoices={color:[['blue','Bleu'],['green','Vert'],['yellow','Jaune'],['pink','Rose']],status:[['fini','Fini'],['classe','Classé'],['en-retard','En retard'],['urgent','Urgent']],priority:[['essentielle','Essentielle'],['prioritaire','Prioritaire'],['important','Important'],['a-faire','À faire'],['futile','Futile'],['inutile','Inutile']]};
...
$('export-selection').onclick=()=>{
 corpusTargets=rows.filter(r=>selected.has(r.key));if(!corpusTargets.length)return;
 const visibleKeys=new Set(visible.map(r=>r.key)),outside=corpusTargets.filter(r=>!visibleKeys.has(r.key)).length;
 $('corpus-scope').textContent=corpusTargets.length+' élément(s) sélectionné(s)'+(outside?' — dont '+outside+' hors des filtres actuels.':'.');
